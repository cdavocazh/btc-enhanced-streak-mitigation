#!/usr/bin/env python3
"""
BTC Strategy Dashboard - All Hours Version
==========================================
Streamlit dashboard for monitoring BTC trading strategies (All Hours version).
...
# Page config
st.set_page_config(
    page_title="BTC Strategy Dashboard - All Hours",
    page_icon="📈",
    layout="wide",
...
# Constants
RESULTS_DIR = os.path.join(os.path.dirname(__file__), 'results_all_hours')
...
def main():
    # Header
    st.title("📈 BTC Strategy Dashboard - All Hours")
    st.markdown("*Real-time monitoring for All Hours trading strategies (0-23 UTC)*")
    ...
    # Calculate historical scores (sample for performance)
    df_scores = df.tail(range_map.get(time_range, 672)).copy()

    # Calculate scores for each row (simplified for performance)
    pos_scores = []
    vol_scores = []
    ...
    # Footer
    st.markdown("---")
    st.caption(f"Data as of: {latest_time} | Total bars: {len(df):,} | Dashboard: All Hours Version")
